...
using PRJ_UnitInfos.API.HealthCheck;
using PRJ_UnitInfos.API.Middleware;
using PRJ_UnitInfos.API;
using Newtonsoft.Json.Converters;
using Swashbuckle.AspNetCore.SwaggerUI;
using System.Text;
using APSharing.WebHttpClient;
using PRJ_UnitInfos.Service;
using Serilog;
...
int timeout = int.Parse(EnvironmentHelper.GetEnvironment("DBConnectionTimeout") ?? "300");

builder.Services.AddDbContext<DbQueryContext>(options => options.UseSqlServer(EnvironmentHelper.GetEnvironment("DBConnectionString"),
    b => { b.MigrationsAssembly("Database.Models"); b.CommandTimeout(timeout); }));
builder.Services.AddDbContext<DatabaseContext>(options => options.UseSqlServer(EnvironmentHelper.GetEnvironment("DBConnectionString"),
    b => { b.MigrationsAssembly("Database.Models"); b.CommandTimeout(timeout); }));
...
var basePath = "api/v1/apcrm-prj-unitinfos-service";
