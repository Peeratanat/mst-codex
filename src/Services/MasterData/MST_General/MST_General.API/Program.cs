...
using MST_General.API.HealthCheck;
using MST_General.API.Middleware;
...
builder.Services.AddHttpClient(Constants.BCAuthClient, client =>
{
    client.BaseAddress = new Uri(Environment.GetEnvironmentVariable("BCAuth_Host"));
    client.DefaultRequestHeaders.Add("Accept", "application/json");
    client.DefaultRequestHeaders.Add("Application-Name", Environment.GetEnvironmentVariable("BCAuthApp_name"));
    client.DefaultRequestHeaders.Add("client_id", Environment.GetEnvironmentVariable("BCAuthClient_id"));
    client.DefaultRequestHeaders.Add("client_secret", Environment.GetEnvironmentVariable("BCAuthClient_secret"));
});
...
var jwtOptions = new JwtOptions
{
    ...
    ExpiryMinutes = int.Parse(Environment.GetEnvironmentVariable("jwt_ExpiryMinutes") ?? "60"),
...
builder.Services
    ...
    .AddPolicyScheme("MultiScheme", "MultiScheme", options =>
    {
        options.ForwardDefaultSelector = ctx =>
            ctx.Request.Headers["Authorization"].FirstOrDefault() is { } header && header.StartsWith("Bearer ") &&
            header.Count(c => c == '.') == 2
    ...
    .AddOAuth2Introspection("OAuth2Introspection", options =>
    {
        ...
        options.ClientId = _env.ApiName;
        options.ClientSecret = _env.ApiSecret;
        options.DiscoveryPolicy.RequireHttps = !_env.IsDevelopment;
    })
...
builder.Services.AddAuthorization(options =>
{
    ...
    options.AddPolicy("MultiSchemePolicy", defaultPolicy);
    options.AddPolicy("CRM.Read", p => p.RequireClaim("scope", "CRM.Read"));
    options.AddPolicy("CRM.Write", p => p.RequireClaim("scope", "CRM.Write"));
});

// ✅ ใช้สำหรับเติม claim เพิ่มหลัง auth (ถ้ามี logic พิเศษ)
builder.Services.AddTransient<IClaimsTransformation, ClaimsTransformationMiddleware>();
...
//        BatchSize = 1000000,             // ลดจาก 3.2MB เพื่อไม่ให้รอนาน
...
var basePath = "api/v1/apcrm-mst-general-service";
