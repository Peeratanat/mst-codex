...
namespace Base.DTOs.SAL
{
	...
	/// </summary>
	public class QuotationPromotionExpensePreBookDTO : BaseDTO
	{
		/// <summary>
		/// ผู้รับผิดชอบ คชจ.
		...
		//public static QuotationSalePromotionExpensePrebookItemDTO CreateFromModel(QuotationSalePromotionExpensePrebook model, Guid quotationUnitPriceID, DatabaseContext db)
		//{
		//    if (model != null)
		//    {
		//        QuotationSalePromotionExpensePrebookItemDTO result = new QuotationSalePromotionExpensePrebookItemDTO()
		//        {
		//            Id = model.ID,
		...
		public static QuotationSalePromotionExpensePrebookItemDTO CreateFromPreBookModel(QuotationSalePromotionExpensePrebook model, Guid quotationUnitPriceID, DatabaseContext db)
		{
			...
			if (model != null)
			{
				QuotationSalePromotionExpensePrebookItemDTO result = new QuotationSalePromotionExpensePrebookItemDTO()
				{
					Id = model.ID,
					ExpenseReponsibleBy = MST.MasterCenterDropdownDTO.CreateFromModel(model.ExpenseReponsibleBy),
					//Name = model.MasterPriceItem != null ? model.MasterPriceItem.Detail : model.PriceName,
					Name = model.PriceName,
					//Amount = Math.Ceiling(model.Amount),
					BuyerPayAmount = Math.Ceiling(model.BuyerAmount),
					SellerPayAmount = Math.Ceiling(model.SellerAmount),
					MasterPriceItem = MST.MasterPriceItemDTO.CreateFromModel(model.MasterPriceItem),
					QuotationID = model.QuotationID,
					MasterPriceItemDescription = model.MasterPriceItemDescription,
					MasterPriceItemSeqNo = model.MasterPriceItemSeqNo
				};

				var item = db.QuotationUnitPriceItems.Include(o => o.PriceUnit).Where(o => o.QuotationUnitPriceID == quotationUnitPriceID && o.MasterPriceItemID == model.MasterPriceItemID).FirstOrDefault();
				if (item != null)
				{
					result.PriceUnitAmount = model.MasterPriceItemID != null ? item.PriceUnitAmount : model.PriceUnitAmount;
					result.PricePerUnitAmount = model.MasterPriceItemID != null ? item.PricePerUnitAmount : model.PricePerUnitAmount;
					result.PriceUnit = model.MasterPriceItemID != null ? MST.MasterCenterDropdownDTO.CreateFromModel(item.PriceUnit) : null;
				}
				else
				{
					result.PriceUnitAmount = model.PriceUnitAmount;
					result.PricePerUnitAmount = model.PricePerUnitAmount;
					//result.PriceUnit = new MST.MasterCenterDropdownDTO();

				}
				result.Amount = Math.Ceiling(model.Amount);
		...
		public async static Task<List<QuotationPromotionExpensePreBookDTO>> CreateDraftFromUnitAsync(Guid unitID, DatabaseContext db, decimal res)
		{
			...
			var results = new List<QuotationPromotionExpensePreBookDTO>();
			var order = 0;
				...
				case "1":
					{
						...
						//var freeText = masterPriceItems.Where(o => o.ID == MasterPriceItemIDs.DocumentFee).ToList();

						#region Meter Water and Elect
						if (waterMaster != null)
						{
							if (unit.WaterMeterPriceID != null)
							{
								var water = new QuotationPromotionExpensePreBookDTO()
								{
						...
						if (electricMaster != null)
						{
							if (unit.ElectricMeterPriceID != null)
							{
								var electric = new QuotationPromotionExpensePreBookDTO()
								{
						...
						if (mortgageMaster != null)
						{
							...
							var mortgage = new QuotationPromotionExpensePreBookDTO()
							{
						...
						if (transferMaster != null)
						{
							...
							var transfer = new QuotationPromotionExpensePreBookDTO()
							{
							...
							if (commonMaster != null)
							{
								...
								var publicFundRate = agreeConfig.PublicFundRate != null ? agreeConfig.PublicFundRate : 0;
								var publicFundMonths = agreeConfig.PublicFundMonths != null ? agreeConfig.PublicFundMonths : 0;

								var common = new QuotationPromotionExpensePreBookDTO();
								common.Name = commonMaster.Detail;
								common.PriceUnitAmount = publicFundMonths;
								common.PriceUnit = MST.MasterCenterDropdownDTO.CreateFromModel(priceUnitMasterCenters.Find(o => o.Key == "3"));

								common.PricePerUnitAmount = publicFundRate * (decimal)(titledeedArea != null ? titledeedArea : unit.SaleArea);
								common.Amount = Math.Ceiling(publicFundRate.Value * publicFundMonths.Value * (decimal)(titledeedArea != null ? titledeedArea : unit.SaleArea));
								common.MasterPriceItem = MST.MasterPriceItemDTO.CreateFromModel(commonMaster);
								common.Order = order++;
								common.ExpenseReponsibleBy = MST.MasterCenterDropdownDTO.CreateFromModel(reponsibleByCustomerMasterCenters);
							...
							{
								var document = new QuotationPromotionExpensePreBookDTO()
								{
				...
				case "1":
					{
						...
						var common = new SalePromotionExpenseDTO()
						{
							...
							PriceUnitAmount = agreeConfig.PublicFundMonths != null ? agreeConfig.PublicFundMonths : 0,
							PriceUnit = MST.MasterCenterDropdownDTO.CreateFromModel(priceUnitMasterCenters.Find(o => o.Key == "3")),
							PricePerUnitAmount = agreeConfig.PublicFundRate != null ? agreeConfig.PublicFundRate * (decimal)(titledeedArea != null ? titledeedArea : unit.SaleArea) : 0,
							Amount = agreeConfig.PublicFundRate != null && agreeConfig.PublicFundMonths != null ?
									 Math.Ceiling(agreeConfig.PublicFundRate.Value * agreeConfig.PublicFundMonths.Value * (decimal)(titledeedArea != null ? titledeedArea : unit.SaleArea)) : 0,
