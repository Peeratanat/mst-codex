...
{
    public class FloorPlanDetailDTO : BaseDTO
    {
        ...
        public static async Task<FloorPlanDetailDTO> CreateFromModelAsync(Unit model, FileHelper fileHelper, DatabaseContext db)
        {
            if (model != null)
            {
                var result = new FloorPlanDetailDTO();
                ...
                var floorplan = await db.FloorPlanImages.Where(o => o.Name == model.FloorPlanFileName).Select(o => o.FileName).FirstOrDefaultAsync();
                var roomplan = await db.RoomPlanImages.Where(o => o.Name == model.RoomPlanFileName).Select(o => o.FileName).FirstOrDefaultAsync();
