    ...
    public static class sqlTranferPromotionDropdownList
    {
        public static string QueryString = @"
              ...
              FROM USR.[User] us
				...
				LEFT JOIN PRM.TransferPromotion tfp 
				ON tfp.BookingID =agr.BookingID AND tfp.IsDeleted=0 
				LEFT JOIN MST.MasterCenter mc ON mc.ID = tfp.TransferPromotionStatusMasterCenterID
              where
				tfp.ID IS NOT NULL 
				AND
                 (r.Code='LCM') 
