    ...
    public static class sqlCancleMemoDropdownList
    {
        public static string QueryString = @"
              ...
              FROM USR.[User] us
                ...
                LEFT JOIN SAL.CancelMemo cm 
                    ON cm.AgreementID = agr.ID
              where
                cm.id is not null 
                and cm.AgreementID is not null 
                and cm.ApproveDate is null 
                and cm.ApproveByUserID is null 
                and cm.IsDeleted = 0
                AND (r.Code='LCM') 
                AND us.EmployeeNo <> 'TEST_LCM' #UserId
