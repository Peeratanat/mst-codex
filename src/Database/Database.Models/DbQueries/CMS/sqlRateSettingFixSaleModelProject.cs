...
{
    public class sqlRateSettingFixSaleModelProject
    {
        ...
        FROM PRJ.Model M
	        ...
	        LEFT OUTER JOIN  (
		        SELECT S1.*
		        FROM CMS.RateSettingFixSaleModel S1 INNER JOIN
		        (
			        SELECT ProjectID,ModelID,MAX(ActiveDate) AS ActiveDate
			        FROM CMS.RateSettingFixSaleModel 
			        WHERE IsActive =1 
				        AND IsDeleted = 0
        ...
        public static List<SqlParameter> QueryFilter(ref string QueryString, Guid? ProjectID, DateTime? ActiveDate,string ModelCode)
        {
            ...
            {
                ParamList.Add(new SqlParameter("@ModelCode", ModelCode));
